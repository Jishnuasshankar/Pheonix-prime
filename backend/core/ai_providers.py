...
class ProviderManager:
    ...
    def select_best_model_sync(self, category: str) -> str:
        """
        Synchronous version of model selection (for streaming start)
        
        Returns just the provider name, not full tuple.
        Used when we need instant provider selection without await.
        ...
        # Simple heuristic selection (fast, no async benchmarking)
        # Prefer Groq for coding (fast), Gemini for reasoning/general
        if category == "coding":
    ...
    # ========================================================================
    # STREAMING SUPPORT (Phase: Real-Time Streaming)
    # ========================================================================
    ...
    async def generate_stream(
        ...
        Enterprise-grade with error handling, health tracking, and fallback.
        ...
        try:
            ...
            logger.info(f"🎬 Starting streaming from {provider_name} (category: {category})")
            
            # Route to appropriate streaming implementation
            if provider_name == "gemini":
                async for chunk in self._stream_gemini(prompt, temperature, max_tokens):
                    tokens_generated += len(chunk.split())
                    yield chunk
            
            elif provider_name == "groq":
                async for chunk in self._stream_groq(prompt, temperature, max_tokens):
                    tokens_generated += len(chunk.split())
                    yield chunk
            
            else:
                # Fallback for providers without streaming: chunk the full response
                logger.warning(f"Provider {provider_name} doesn't support streaming, using fallback")
                ...
                # Simulate streaming by chunking (word-by-word for smooth UX)
            ...
            logger.info(
                f"✅ Streaming complete: {provider_name}, "
                f"{tokens_generated} tokens, {response_time_ms:.0f}ms"
            )
            ...
            logger.error(f"❌ Streaming generation failed for {provider_name}: {e}")
            raise ProviderError(f"Streaming failed: {str(e)}") from e
    ...
    async def _stream_gemini(
        ...
        Uses official google.generativeai SDK with streaming support.
        """
        try:
            import google.generativeai as genai
            ...
            raise ProviderError(f"Gemini streaming failed: {str(e)}") from e
    ...
    async def _stream_groq(
        ...
        Uses official Groq SDK with async streaming support.
        """
        try:
            from groq import AsyncGroq
            ...
            raise ProviderError(f"Groq streaming failed: {str(e)}") from e
    
    def estimate_cost(self, provider_name: str, estimated_tokens: float) -> float:
        """
        Estimate cost for a given number of tokens
        
        Args:
            provider_name: Provider name
            estimated_tokens: Estimated tokens (input + output)
            
        Returns:
            Estimated cost in USD
        """
        try:
            # Default costs (USD per 1M tokens) - fallback
            default_costs = {
                "gemini": 0.075,  # Gemini 2.5 Flash average
                "groq": 0.05,     # Groq average
                "openai": 0.15,   # GPT-4 average
                "anthropic": 0.30 # Claude average
            }
            
            cost_per_1m = default_costs.get(provider_name, 0.10)
